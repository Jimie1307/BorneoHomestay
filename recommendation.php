...
<?php
$sql = "SELECT * FROM homestay WHERE homestayImg !='' AND rating > 3 order by rating DESC";
$hs_list = $conn->query($sql);
if ($hs_list->num_rows > 0) {
while($row = $hs_list->fetch_assoc()) {
?>

<div class="product-image"><img style="margin-bottom: 3%; position: right;" src="<?php echo $row["homestayImg"]; ?>"></img> <p> ⭐️ <?php echo $row['rating']; ?> </p> <a class="index" style=" padding: 6px; margin-bottom: 3%;" href="homestayView.php?hsID=<?php echo urlencode( $row["homestayName"]) ?>"> <?php echo $row["homestayName"]; ?></a> </div><br>
